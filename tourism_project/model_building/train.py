...
mlflow.set_experiment("Tourism_Package_Prediction_Experiment")     # complete the code to set the MLflow experiment name (same as the dev experimentation cell)
...
numeric_features = ["Age", "CityTier", "DurationOfPitch", "NumberOfPersonVisiting",
    ...
    "MonthlyIncome"]   # complete the code to list all numerical feature names (same as in prep.py)

categorical_features = ["TypeofContact", "Occupation", "Gender", "MaritalStatus",
    "ProductPitched", "Designation"]   # complete the code to list all categorical feature names (same as in prep.py)
...
param_grid = {
    ...
    'xgbclassifier__colsample_bytree': [0.7, 0.8, 0.9],    # Fraction of features sampled when building each tree.
    'xgbclassifier__colsample_bylevel': [0.7, 0.8, 0.9],   # Fraction of features sampled at each tree level.
    'xgbclassifier__learning_rate': [0.05, 0.1, 0.2],       # Step size used during boosting. Smaller values may improve generalization but require more trees.
    'xgbclassifier__reg_lambda': [0.1, 1, 10],          # L2 regularization strength. Higher values help reduce overfitting.
}
